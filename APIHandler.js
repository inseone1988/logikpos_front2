...
const api = {
    ...
    'moneyCount.save': async (e, a) => {
        let c = await CashDrawer.findOne({where: {open: true}});
        if (c) {
            let r = {};
            //Load orders
            let o = await Order.findAll({
                ...
                include: [Order.hasMany(OrderItem), Order.hasMany(Payment)]
            });
            r.orders = o;
            c.difference = Number(a.money) - Number(c.currentBalance);
            await c.save();
            r.cashDrawer = c;
        }
    }
